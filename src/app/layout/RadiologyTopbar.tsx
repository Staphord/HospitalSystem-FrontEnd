...
import { getRadiologyPageTitle } from '@/app/layout/radiologyNavUtils'
...
export function RadiologyTopbar() {
  const { user } = useAuth()
    ...
    <header className="h-12 bg-surface-white border-b border-border-subtle flex justify-between items-center px-lg z-40 shrink-0">
      <div className="flex items-center min-w-0">
        <h1 className="font-headline-md text-headline-md font-semibold text-on-surface truncate m-0">
          {getRadiologyPageTitle(location.pathname)}
        </h1>
      </div>
      ...
      <div className="flex items-center gap-md shrink-0">
        ...
        <div className="flex items-center bg-[#42526E]/10 px-sm py-xs rounded-full gap-xs">
          <span className="w-1.5 h-1.5 bg-success rounded-full" />
          <span className="font-label-md text-label-md text-secondary whitespace-nowrap">Radiology</span>
        </div>
