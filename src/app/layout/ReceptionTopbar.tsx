...
function formatReceptionDateTime(): string {
...
function getReceptionPageTitle(path: string): string {
  const p = path.toLowerCase()
  if (p === '/dashboard' || p.endsWith('/dashboard')) return 'My Dashboard'
  if (p.includes('/reception/register')) return 'Register Patient'
  if (p.includes('/reception/search')) return 'Patient Search'
  if (p.includes('/reception/queue')) return 'Queue Management'
  if (p.includes('/billing')) return 'Payment & Insurance'
...
export function ReceptionTopbar() {
  const { user } = useAuth()
  const location = useLocation()
  const [showNotificationBadge, setShowNotificationBadge] = useState(true)
  const [dateTime, setDateTime] = useState(formatReceptionDateTime)
  const displayName = user?.full_name || user?.username || 'User'

  useEffect(() => {
    setDateTime(formatReceptionDateTime())
    const interval = setInterval(() => setDateTime(formatReceptionDateTime()), 60_000)
    ...
    <header className="h-12 bg-surface-white border-b border-border-subtle flex justify-between items-center px-lg z-40 shrink-0">
      <div className="flex items-center min-w-0">
        <h1 className="font-headline-md text-headline-md font-semibold text-on-surface truncate m-0">
          {getReceptionPageTitle(location.pathname)}
        </h1>
      </div>

      <div className="flex items-center gap-md shrink-0">
        <div className="text-[#737685] font-body-sm text-body-sm hidden md:block">
          {dateTime}
        </div>
        <Link
          to="/notifications"
        ...
        <div className="flex items-center bg-success/10 text-success px-sm py-xs rounded-full gap-xs border border-success/20">
          ...
          <span className="font-label-md text-label-md whitespace-nowrap">Reception Active</span>
        </div>
